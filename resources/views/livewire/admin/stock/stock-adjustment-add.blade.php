    ...
    <div class="page-body">
        <div class="container-fluid">
            <div class="row gx-3 mb-3 align-items-center">
                <div class="col">
                    <h5 class="mb-0">{{ __('main.add_stock_adjustment') }}</h5>
                </div>
                <div class="col-auto">
                                        ...
                                        @elseif($material_query != '' && count($materials) == 0)
                                            <ul class="list-group position-absolute ">
                                                <li id="no-mat" class="list-group-item hover-disabled">
                                                    {{ __('main.no_materials_found') }}</li>
                                            </ul>
                                        @endif
                                    </div>
                                    <div class="col-lg-2">
                                        <input class="form-control" value="{{ $date }}" required type="date"
                                            wire:model="date" />
                                    </div>
                                </div>
                            </div>
                                    ...
                                    <tbody>
                                        @foreach ($cartItems as $key => $item)
                                            @php
                                                ...
                                                $iteminbills = \App\Models\InvoiceDetail::where('type', 2)
                                                    ->where('item_id', $item['id'])
                                                    ->sum('quantity');
                                                $iteminadjustadd = \App\Models\StockAdjustmentDetail::where('material_id', $item['id'])
                                                    ->where('type', 2)
                                                    ->sum('quantity');
                                                $iteminadjustsub = \App\Models\StockAdjustmentDetail::where('material_id', $item['id'])
                                                    ->where('type', 1)
                                                    ->sum('quantity');
                                                $itemtransfer = \App\Models\StockTransferDetail::where('material_id', $item['id'])->sum('quantity');
                                                $salesreturn = \App\Models\SalesReturnDetail::where('item_id', $item['id'])
                                                    ->whereType(2)
                                                    ->sum('quantity');
                                            ...
                                            <tr>
                                                ...
                                                <td class="w-table-20">
                                                    <div class="input-group">
                                                        <input
                                                            class="form-control @error('qty.' . $key) is-invalid @endif @if ($notenoughstock == true) is-invalid @endif input-sm-2" required wire:model="qty.{{ $key }}" type="number" value="100" placeholder="Enter Value" />
                                ...
                                <div class="row g-3 mb-0 align-items-center justify-content-between">
                                    <div class="col-lg-4 col-12">
                                        <div class="row align-items-center mb-0">
                                            ...
                                            <div class="col fw-bolder text-secondary">{{ $total_items }}
                                                {{ __('main.items') }}</div>
                                        </div>
                                    </div>
                                    <div class="col-lg-4 col-12">
                                        <div class="row align-items-center gx-3">
                                            <div class="col-6">
                                                <button class="btn btn-secondary w-100"
                                                    ...
                                                    type="reset">{{ __('main.clear_all') }}</button>
                                            </div>
                                            <div class="col-6">
                                                <button class="btn btn-primary w-100" wire:click.prevent="save"
                                                    wire:loading.class="disabled" wire:target="save" type="submit">
